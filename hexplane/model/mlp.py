...
class SceneFlow_MLP(torch.nn.Module):
    """
    SceneFlow_MLP module with potential input including time position encoding(PE): t_pe, feature PE: fea_pe, 3D position PE: pos_pe,
    view direction PE: view_pe.
    ...
    def __init__(
        ...
        for _ in range(n_layers - 2):
            layers += [torch.nn.Linear(featureC, featureC), torch.nn.ReLU(inplace=True)]
        outChanel = 3 * taylorOrder
        #layers += [torch.nn.Linear(featureC, outChanel)]
        self.mlp = torch.nn.Sequential(*layers)
        self.sf_linear = torch.nn.Linear(featureC, outChanel)
        self.prob_linear = torch.nn.Linear(featureC, 1)

        if zero_init:
            torch.nn.init.constant_(self.sf_linear.weight, 0)
            torch.nn.init.constant_(self.sf_linear.bias, 0)
            torch.nn.init.constant_(self.prob_linear.bias, 0)
    ...
    def forward(
        ...
        prob = torch.nn.functional.sigmoid(self.prob_linear(feat))
        sf = self.sf_linear(feat)

        return [sf, prob]

class SceneFlow_MLP_feat(torch.nn.Module):
    """
    SceneFlow_MLP_feat module with potential input including time position encoding(PE): t_pe, feature PE: fea_pe, 3D position PE: pos_pe,
    view direction PE: view_pe.
