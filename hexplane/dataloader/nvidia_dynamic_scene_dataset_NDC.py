...
class Nvidia_NDC_Dataset(Dataset):
    def __init__(
        self,
        datadir,
        split="train",
        ...
        self.image_dir = os.path.join(self.root_dir, 'images_*x288')
        self.split = split
        self.downsample = 1.0
    ...
    def load_meta(self):
        ...
        sc = 1. if self.bd_factor is None else 1./(np.percentile(self.near_fars[:, 0], 5) * self.bd_factor)
        poses[:,:3,3] *= sc
        self.near_fars *= sc

        images = glob.glob(os.path.join(self.image_dir, "*.png"))
        images.sort() # sort the list; important
        if len(images) != poses_arr.shape[0]:
            print("There is no matching images... use sample image to set img_wh")

            # update image size parameter
            image0 = cv2.imread(os.path.join(self.root_dir, "sample.png"))
        ...
        # optical flow paths
        fwd_paths = sorted(glob.glob(os.path.join(self.root_dir, 'flow_i1', '*_fwd.npz')))
        bwd_paths = sorted(glob.glob(os.path.join(self.root_dir, 'flow_i1', '*_bwd.npz')))
        ...
        if len(images) != poses_arr.shape[0]:
            print("[warning] There is no matching images... quitting loading dataset.")
            return
        ...
        if self.split == "train":
            ...
            count = 24
            camera_cnt = 12 if self.split == "full_test" else 1
            for camera_i in range(0, camera_cnt):
                video_times = torch.tensor([img_i / (count - 1) for img_i in range(24) if img_i % 12 != camera_i])
            ...
            all_imgs = torch.reshape(all_imgs, [camera_cnt, 22, H*W, 3])
