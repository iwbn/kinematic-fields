...
class Nvidia_NDC_Dataset_Shorter(Dataset):
    def __init__(
        self,
        datadir,
        split="train",
    ...
    def load_meta(self):
        ...
        assert len(images) == poses_arr.shape[0]//2
        ...
        if self.split == "train":
            ...
            for camera_i in range(0, camera_cnt):
                video_times = torch.tensor([img_i / (count - 1) for img_i in range(23) if img_i % 12 != camera_i])
                if len(video_times) > 21:
            ...
            all_imgs = torch.reshape(all_imgs, [camera_cnt, 21, H*W, 3])
