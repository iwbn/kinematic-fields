...
def evaluation(
    ...
    metrics_stime = {k: [] for k in metrics_list}
    metrics_ntime = {k: [] for k in metrics_list}
    ...
    for camera_i, img_i in tqdm_:
        ...
        if savePath is not None and os.path.exists(f"{out_path}/{out_img_base}.png"):
            rgb_map = cv2.imread(f"{out_path}/{out_img_base}.png")[:, :, ::-1] / 255.

            rgb_map = torch.FloatTensor(rgb_map)
            res_sf = {}
        ...
        if img_i % 2 == 0:
            metrics = metrics_stime
        else:
            metrics = metrics_ntime
    ...
    with open(f"{savePath}/{prefix}mean_stime.txt", "w") as f:
        res = ""
        for k in metrics_list:
            res += f"{k}: {np.mean(np.asarray(metrics_stime[k]))}, "
        res = res[:-2] + "\n"
        ...
        for i in range(len(metrics_stime[metrics_list[0]])):
            res = "Index %d, " % i
            for k in metrics_list:
                res += f"{k}: {metrics_stime[k][i]}, "
            res = res[:-2] + "\n"
            f.write(
                res
            )
    
    with open(f"{savePath}/{prefix}mean_ntime.txt", "w") as f:
        res = ""
        for k in metrics_list:
            res += f"{k}: {np.mean(np.asarray(metrics_ntime[k]))}, "
        res = res[:-2] + "\n"
        ...
        for i in range(len(metrics_ntime[metrics_list[0]])):
            res = "Index %d, " % i
            for k in metrics_list:
                res += f"{k}: {metrics_ntime[k][i]}, "
            res = res[:-2] + "\n"
            f.write(
                res
            )
